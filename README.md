...
|`rewrite`|bool|Значение true или false, означающее стоит ли перезаписывать генерируемый файл, если файл с таким именем по сохраняемому пути уже существует, если ключ не указан, то будет запрошено во время выполнения|
...
#### Шаблонные значения
- Все значения по ключу **Custom** описаного ниже для файлов шаблонов
- Все значения по ключу **Answers** описаного ниже для файлов шаблонов
- Все значения по ключу **ModuleInfo** описаного ниже для файлов шаблонов
...
_Например в шаблоне описать вопрос_
```
questions:
  - {key: versionApi,
    text: "Enter API version: ",
...
### 1.3.4
  ...
  - Добавлен параметр [--force, --f](#setup) к команде [setup](#setup), насильно перезапрашивающий данные конфигурации
